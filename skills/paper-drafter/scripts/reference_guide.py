...
def _validate_route(index: dict[str, Any], domain_name: str | None, phase_id: str | None, section_id: str | None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    candidates = _candidate_block(index)
    missing = [
        name
        for name, value in (
            ("canonical-domain-name", domain_name),
            ("drafting-phase", phase_id),
...
def command_list(index: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("## Drafting phases")
    for phase_id, phase in sorted(phases.items()):
        lines.append(f"- `{phase_id}`: {phase.get('label', phase_id)}")
    lines.append("")
...
def command_nav(index: dict[str, Any], domain_name: str | None, phase_id: str | None, section_id: str | None) -> str:
    domain, phase, section = _validate_route(index, domain_name, phase_id, section_id)
    assert domain_name is not None
    assert phase_id is not None
    assert section_id is not None
    ...
    for heading in _as_str_list(phase.get("general_headings", []), "drafting_phase.general_headings"):
        add(general_ref, heading, "general phase", "general")
    ...
    phase_headings = _require_mapping(domain, "phase_headings").get(phase_id, [])
    for heading in _as_str_list(phase_headings, "domain.phase_headings"):
        add(domain_ref, heading, "domain phase", domain_name)
    ...
    lines = [
        ...
        f"- Drafting phase: {phase.get('label', phase_id)} (`{phase_id}`)",
    ...
    for record in records:
        if record["source"] == "taxonomy":
            show_command = f"uv run --project=\"$HOME/.ar\" --locked -- python paper-drafter/scripts/reference_guide.py taxonomy --domain {record['show_domain']}"
        else:
            show_command = f"uv run --project=\"$HOME/.ar\" --locked -- python paper-drafter/scripts/reference_guide.py show --domain {record['show_domain']} --heading \"{record['heading']}\""
        lines.extend(
            [
    ...
    lines.extend(
        [
            "",
            "## Use rules",
            "- This command is an index navigator only; it does not summarize, rewrite, or draft paper content.",
            "- Do not treat these headings as manuscript evidence or user-approved writing plans.",
            "- Use `show` only for headings that are needed for the current drafting blocker or writing decision.",
...
def _collect_index_headings(index: dict[str, Any]) -> dict[str, set[str]]:
    ...
    for phase_id, phase in _require_mapping(index, "drafting_phases").items():
        add(str(general.get("ref", "general")), _as_str_list(phase.get("general_headings", []), f"drafting_phases.{phase_id}.general_headings"))
    ...
    for domain_id, domain in _domains(index).items():
        ...
        for phase_id, headings in _require_mapping(domain, "phase_headings").items():
            add(domain_ref, _as_str_list(headings, f"domains.{domain_id}.phase_headings.{phase_id}"))
...
def command_check(index: dict[str, Any]) -> str:
    if index.get("schema_version") != 3:
        raise GuideError("reference_index.json schema_version must be 3")
...
def command_guide_deprecated() -> str:
    return "\n".join(
        [
            "# Deprecated command",
            "",
            "`guide` no longer generates drafting instructions.",
            "",
            ...
            "- `nav --domain <canonical-domain-name> --drafting-phase <phase-id> --paper-section <section-id>`: find candidate reference headings.",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Navigate paper-drafter reference documents by canonical domain name without summarizing, rewriting, or drafting from them.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("list", help="List canonical domain names, drafting phases, and paper sections.")
    ...
    guide.add_argument("--drafting-phase")
    guide.add_argument("--paper-section")

    nav = subparsers.add_parser("nav", help="Return candidate reference headings for one drafting route.")
    nav.add_argument("--domain", required=True, help="Canonical domain name, e.g. computational-algorithm-data-driven or general.")
    nav.add_argument("--drafting-phase", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "list":
            ...
            output = command_nav(index, args.domain, args.drafting_phase, args.paper_section)
