...
- 完成阶段一写库后，立即运行 `gate-and-render` 核心脚本：
  `conda run --no-capture-output -n DataProcessing python -u revision-master/scripts/gate_and_render_workspace.py --artifact-root <ARTIFACT_ROOT>`
- 运行后先读取：
  - `instruction_payload.resume_packet`
  - `01-agent-resume.md`
