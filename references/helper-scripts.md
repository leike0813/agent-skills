...
这份文档描述 `revision-master` 当前运行时使用的正式脚本集合与职责边界。
...
- 路径：`revision-master/scripts/detect_main_tex.py`
...
- 路径：`revision-master/scripts/init_artifact_workspace.py`
...
- 路径：`revision-master/scripts/gate_and_render_workspace.py`
- 正式称呼：`gate-and-render` 核心脚本
- 作用：
  - 读取 `revision-master.db`
  - 检查状态机门禁
  - 重渲染只读视图
...
- 路径：`revision-master/scripts/capture_revision_action.py`
...
- 路径：`revision-master/scripts/commit_revision_round.py`
