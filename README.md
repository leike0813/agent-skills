...
| tag-regulator | 在严格受控词表（`valid_tags`）约束下，利用 LLM 语义理解对 Zotero tags 进行规范化，并可基于元数据推断（infer_tag）标签。作为 `$tag-regulator` 被调用，从 prompt payload 读取 `metadata`/`input_tags`/`valid_tags`/`infer_tag`，并支持 `tag_note_language` 控制建议说明语言，stdout 必须且只能输出一个 JSON 对象。 |
