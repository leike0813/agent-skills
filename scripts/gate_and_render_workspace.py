...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the gate-and-render core script for a DB-first revision-master artifact workspace.")
...
def build_repair_sequence(
    ...
    target_map = {
        "workflow_state": ["revision-master.db"],
        "resume_brief": ["revision-master.db", AGENT_RESUME_MD],
        "resume_open_loops": ["revision-master.db", AGENT_RESUME_MD],
        "resume_recent_decisions": ["revision-master.db", AGENT_RESUME_MD],
        "resume_must_not_forget": ["revision-master.db", AGENT_RESUME_MD],
        "manuscript_summary": ["revision-master.db", MANUSCRIPT_SUMMARY_MD],
        "raw_review_threads": ["revision-master.db", RAW_REVIEW_THREADS_MD],
        "atomic_comments": ["revision-master.db", ATOMIC_COMMENTS_MD],
        "raw_thread_atomic_links": ["revision-master.db", THREAD_TO_ATOMIC_MAPPING_MD],
        "review_comment_source_documents": ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD],
        "raw_thread_source_spans": ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD],
        "review_comment_coverage_segments": ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD],
        "review_comment_coverage_segment_comment_links": ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD],
        "atomic_comment_state": ["revision-master.db", ATOMIC_WORKBOARD_MD],
        "atomic_comment_target_locations": ["revision-master.db", ATOMIC_WORKBOARD_MD],
        "atomic_comment_analysis_links": ["revision-master.db", ATOMIC_WORKBOARD_MD],
        "strategy_cards": ["revision-master.db", STRATEGY_CARD_DIR],
        "strategy_action_manuscript_execution_items": ["revision-master.db", STRATEGY_CARD_DIR],
        "comment_response_drafts": ["revision-master.db", STRATEGY_CARD_DIR],
        "comment_completion_status": ["revision-master.db", FINAL_CHECKLIST_MD],
        "comment_blockers": ["revision-master.db", STRATEGY_CARD_DIR],
        "workspace_manuscript_copies": ["revision-master.db", str(MANUSCRIPT_COPY_ROOT)],
        "revision_plan_actions": ["revision-master.db", REVISION_GUIDE_MD, REVISION_EXECUTION_GRAPH_MD],
        "revision_plan_dependencies": ["revision-master.db", REVISION_EXECUTION_GRAPH_MD],
        "revision_action_logs": ["revision-master.db", REVISION_ACTION_LOG_MD],
        "revision_action_log_plan_links": ["revision-master.db", REVISION_ACTION_LOG_MD],
        "revision_action_log_thread_links": ["revision-master.db", REVISION_ACTION_LOG_MD, RESPONSE_COVERAGE_MATRIX_MD],
        "revision_action_log_entries": ["revision-master.db", REVISION_ACTION_LOG_MD],
        "response_thread_resolution_links": ["revision-master.db", RESPONSE_LETTER_OUTLINE_MD, RESPONSE_TABLE_PREVIEW_MD],
        "style_profiles": ["revision-master.db", STYLE_PROFILE_MD],
        "style_profile_rules": ["revision-master.db", STYLE_PROFILE_MD],
        "action_copy_variants": ["revision-master.db", ACTION_COPY_VARIANTS_MD],
        "selected_action_copy_variants": ["revision-master.db", ACTION_COPY_VARIANTS_MD],
        "response_thread_rows": ["revision-master.db", RESPONSE_TABLE_PREVIEW_MD, RESPONSE_TABLE_PREVIEW_TEX],
        "export_patch_sets": ["revision-master.db", EXPORT_PATCH_PLAN_MD],
        "export_patches": ["revision-master.db", EXPORT_PATCH_PLAN_MD],
        "export_artifacts": ["revision-master.db", FINAL_CHECKLIST_MD],
        "supplement_suggestion_items": ["revision-master.db", SUPPLEMENT_SUGGESTION_PLAN_MD],
        "supplement_suggestion_intake_links": ["revision-master.db", SUPPLEMENT_SUGGESTION_PLAN_MD],
        "supplement_intake_items": ["revision-master.db", SUPPLEMENT_INTAKE_PLAN_MD],
        "supplement_landing_links": ["revision-master.db", SUPPLEMENT_INTAKE_PLAN_MD],
    }
    instruction_map = {
    ...
    for index, ((artifact, _comment_id), details) in enumerate(ordered, start=1):
        repairs.append(
            make_action(
                ...
                target_map.get(artifact, ["revision-master.db"]),
    ...
    if current_stage == "stage_3" and pending:
        blocked.append(
            make_action(
                ...
                ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD, THREAD_TO_ATOMIC_MAPPING_MD],
    ...
    if current_stage == "stage_4" and pending:
        blocked.append(
            make_action(
                ...
                ["revision-master.db", ATOMIC_WORKBOARD_MD],
        ...
        if pending:
            blocked.append(
                make_action(
                    ...
                    ["revision-master.db", f"{STRATEGY_CARD_DIR}/{active_comment_id}.md", SUPPLEMENT_SUGGESTION_PLAN_MD],
                    recipe_id=None,
                )
            )
        if blockers or comment_blocker_map.get(active_comment_id):
            blocked.append(
                make_action(
                    ...
                    ["revision-master.db", SUPPLEMENT_INTAKE_PLAN_MD],
    ...
    if current_stage != "stage_6" or export_not_ready:
        blocked.append(
            make_action(
                ...
                ["revision-master.db", FINAL_CHECKLIST_MD, RESPONSE_TABLE_PREVIEW_MD, RESPONSE_TABLE_PREVIEW_TEX],
        ...
        if current_stage == "stage_3":
            ...
            return [
                make_action(
                    ...
                    reason,
                    ["revision-master.db", REVIEW_COMMENT_COVERAGE_MD, RAW_REVIEW_THREADS_MD, THREAD_TO_ATOMIC_MAPPING_MD],
                    recipe_id="recipe_stage3_clear_coverage_confirmation",
                )
            ]
        if current_stage == "stage_4":
            return [
                make_action(
                    ...
                    ["revision-master.db", ATOMIC_WORKBOARD_MD, THREAD_TO_ATOMIC_MAPPING_MD],
                    recipe_id="recipe_stage4_set_pending_confirmations",
                )
            ]
        if current_stage == "stage_5" and active_comment_id:
    ...
    if current_stage == "stage_1":
        return [
            make_action(
                ...
                ["revision-master.db", MANUSCRIPT_SUMMARY_MD],
    ...
    if current_stage == "stage_2":
        return [
            make_action(
                ...
                ["revision-master.db", RAW_REVIEW_THREADS_MD, ATOMIC_COMMENTS_MD, THREAD_TO_ATOMIC_MAPPING_MD],
    ...
    if current_stage == "stage_4":
        ...
        return [
            make_action(
                ...
                ["revision-master.db", STRATEGY_CARD_DIR, SUPPLEMENT_SUGGESTION_PLAN_MD],
            ...
            if not strategy_confirmed:
                ...
                actions.append(
                    make_action(
                        ...
                        ["revision-master.db", f"{STRATEGY_CARD_DIR}/{active_comment_id}.md", SUPPLEMENT_SUGGESTION_PLAN_MD, SUPPLEMENT_INTAKE_PLAN_MD],
            ...
            if other_comment is not None:
                actions.append(
                    make_action(
                        ...
                        ["revision-master.db", ATOMIC_WORKBOARD_MD, STRATEGY_CARD_DIR],
                        recipe_id="recipe_stage5_set_active_comment",
                    )
                )
            return actions
        ...
        if next_comment is None and atomic_state_map and all(str(row["status"]) == "done" for row in atomic_state_map.values()):
            return [
                make_action(
                    ...
                    ["revision-master.db", REVISION_GUIDE_MD, REVISION_EXECUTION_GRAPH_MD, SUPPLEMENT_INTAKE_PLAN_MD],
        ...
        if missing_style:
            return [
                make_action(
                    ...
                    localization.msg("gate.allowed.enter_stage_6.instruction"),
                    localization.msg("gate.allowed.enter_stage_6.reason"),
                    ["revision-master.db", STYLE_PROFILE_MD],
        ...
        if not revision_plan_rows:
            return [
                make_action(
                    ...
                    localization.msg("gate.allowed.enter_stage_6.instruction"),
                    localization.msg("gate.allowed.enter_stage_6.reason"),
                    ["revision-master.db", REVISION_GUIDE_MD, REVISION_EXECUTION_GRAPH_MD],
                    recipe_id="recipe_stage5_build_revision_backlog",
                )
            ]
        unresolved_actions = [
        ...
        if unresolved_actions:
            return [
                make_action(
                    ...
                    ["revision-master.db", REVISION_GUIDE_MD, REVISION_EXECUTION_GRAPH_MD, REVISION_ACTION_LOG_MD],
        ...
        if len(response_thread_row_map) < len(raw_thread_map):
            return [
                make_action(
                    ...
                    ["revision-master.db", RESPONSE_COVERAGE_MATRIX_MD, RESPONSE_TABLE_PREVIEW_MD, RESPONSE_TABLE_PREVIEW_TEX],
                    recipe_id="recipe_stage6_refresh_response_rows",
                )
            ]
        missing_thread_logs = [
